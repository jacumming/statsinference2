...
plot(y=volcano$Small,x=volcano$Year,ty='l', ylim=c(0,50))
...
hist(volcano$Medium)
## difficult to tell, no strong evidence against
...
## To draw the plot all in one go:
plot(x=lambdavals, y=cnjPrior,ty='l',col='red',ylim=c(0,0.02), 
...
abline(v=T/n, col='blue', lty=2)
lines(x=lambdavals,y=postDirect,col='green')
lines(x=lambdavals,y=postConjugate,col='orange',lty=3)
abline(v=aPost/bPost,col='green', lty=2)
...
"gammaPoisson3" <- function(a,b,T,n){
  ...
  sig2 <- 1/(n/(T/n)) # var = 1/[I(lambda hat)]
