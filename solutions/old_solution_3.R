...
plot(y=w,x=d,xlim=c(0,5.5),ylim=c(0,100))
guess.b0 <- 24
guess.b1 <- 14
abline(a=guess.b0,b=guess.b1,col='blue')
...
guess.what <- guess.b0 + guess.b1*d
guess.resid <- w - guess.what
guess.Q <- sum(guess.resid^2)
...
dbar <- mean(d)
sdd <- sum((d-dbar)^2)
sdw <- sum((d-dbar)*(w-wbar))
## OLS estimates
beta1hat <- sdw/sdd
beta0hat <- wbar-beta1hat*dbar
## the plot
plot(y=w, x=d, xlim=c(0,5.5), ylim=c(0,100))
abline(a=beta0hat, b=beta1hat, col='blue')
...
## get the standard error of the intercept (col 2, row 1)
se.beta0 <- summary(model)$coefficients[1,2]
## find yhat
w.hat <- beta0hat + beta1hat * 3
## standard error of yhat
se.what <- se * sqrt(1/n + (3-dbar)^2/sdd)
## 95% CI for yhat
w.hat + c(-1,1)* qt(0.975,n-2) * se.what
## standard error of prediction
se.pred <- se * sqrt(1 + 1/n + (3-dbar)^2/sdd)
## 95% PI for y
w.hat + c(-1,1)* qt(0.975,n-2) * se.pred
